...
srun python tools/run_distributed_engines_25K.py \
  hydra.verbose=true \
  ...
  config.OPTIMIZER.num_epochs=35 \
  config.OPTIMIZER.param_schedulers.lr.lengths="[0.025, 0.975]" \
  config.CHECKPOINT.DIR="/scratch/tjian/PythonProject/deep_plastic_Flux_SSL/checkpoint/train_weights/RN50_25K_300e/vissl_265_to_300e" \
  config.HOOKS.TENSORBOARD_SETUP.USE_TENSORBOARD=False \
  config.WEIGHTS_INIT.PARAMS_FILE="/scratch/tjian/PythonProject/deep_plastic_Flux_SSL/checkpoint/train_weights/RN50_25K_300e/vissl_265/model_phase265.torch" \
