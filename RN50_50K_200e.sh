...
srun python tools/run_distributed_engines_50K.py \
  hydra.verbose=true \
  ...
  config.OPTIMIZER.num_epochs=70 \
  config.OPTIMIZER.param_schedulers.lr.lengths="[0.05, 0.95]" \
  config.CHECKPOINT.DIR="/scratch/tjian/PythonProject/deep_plastic_Flux_SSL/checkpoint/train_weights/RN50_50K_200e/vissl_130_to_200" \
  config.HOOKS.TENSORBOARD_SETUP.USE_TENSORBOARD=False \
  config.WEIGHTS_INIT.PARAMS_FILE="/scratch/tjian/PythonProject/deep_plastic_Flux_SSL/checkpoint/train_weights/RN50_50K_200e/vissl_0_to_130e/model_phase130.torch" \
