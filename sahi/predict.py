...
import pandas as pd  # Import pandas for Excel file output
...
def predict(
    ...
    **kwargs,
    ...
    # Initialize the total detections counter
    total_detections = 0
    image_detection_data = []  # List to store (filename, number of detections)
    ...
    save_dir = Path(increment_path(Path(project), exist_ok=False))  # increment run
    crop_dir = save_dir / "crops"
    visual_dir = save_dir / "pred_images"
    ...
    for ind, image_path in enumerate(
        ...
        # Output the number of detections for this image/frame
        num_detections = len(object_prediction_list)
        tqdm.write(f"Number of detections in one image: {num_detections}")
        
        # Increment total detections
        total_detections += len(object_prediction_list)

        # Append the filename and number of detections to the list
        image_detection_data.append([relative_filepath, num_detections])
    ...
    # export coco results
    if dataset_json_path:
        save_path = str(save_dir / "results.json")
    ...
    # Export the filename and detection count to an Excel file using pandas
    df = pd.DataFrame(image_detection_data, columns=['Filename', 'Number of Detections'])
    excel_output_path = str(save_dir / "detection_results.xlsx")
    df.to_excel(excel_output_path, index=False)
    print(f"Detection results saved to {excel_output_path}")

    # After all images are processed, print the total number of detections
